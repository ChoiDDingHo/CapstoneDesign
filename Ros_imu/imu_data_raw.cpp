...
double invSampleFreq = 1.0 / 400.0; // inverse sample frequency
ros::Time last_time;
...
void publish_imu_data(rs2::pipeline& camera_pipe) {
    ...
    while (ros::ok()) {
        ...
        MadgwickAHRSupdateIMU(accel_data_array[0], accel_data_array[1], accel_data_array[2],
                              gyro_data_array[0], gyro_data_array[1], gyro_data_array[2]);
        ...
        imu_msg.linear_acceleration.x = accel_data_array[0];
        imu_msg.linear_acceleration.y = accel_data_array[1];
        imu_msg.linear_acceleration.z = accel_data_array[2];

        imu_msg.angular_velocity.x = gyro_data_array[0];
        imu_msg.angular_velocity.y = gyro_data_array[1];
        imu_msg.angular_velocity.z = gyro_data_array[2];
